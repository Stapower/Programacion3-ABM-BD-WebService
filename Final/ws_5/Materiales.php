...
class Materiales
{
	...
	public $id;
	 ...
	 public function InsertarMaterial()
	 {
        try{
				...
				$consulta =$objetoAccesoDato->RetornarConsulta('INSERT INTO mascotas (nombre,raza, tipo) values (?, ?, ?)');
                                
				$consulta->execute(array($this->nombre, $this->precio, $this->tipo));
	...
	public function ModificarMaterial()
	{
          ...
          {
            $objetoAccesoDato = AccesoDatos::dameUnObjetoAcceso(); 
			$consulta = $objetoAccesoDato->RetornarConsulta("UPDATE mascotas SET nombre ='$this->nombre', raza= '$this->precio', tipo = '$this->tipo' WHERE id='$this->id'");
            $consulta->execute();
          }
         catch(Exception $e)
   		{ 
